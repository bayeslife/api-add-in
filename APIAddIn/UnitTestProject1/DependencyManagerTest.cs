...
namespace UnitTestProject1
{
    ...
    public class DependencyManagerTest
    {
        ...
        {            
            ...
            List<string> dependencies = dependencyManager.getDependencies();
            Assert.IsTrue(dependencies.IndexOf("Structure") < dependencies.IndexOf("CoverageComponent"));
        ...
        public void TestManageSchemaDependencies()
        {
            ...
            Assert.AreEqual("class4", ordered[0]);
            Assert.AreEqual("class5", ordered[1]);

            c.setDependency("class6", "class3");

            Assert.AreEqual("class6", ordered[2]);
            Assert.AreEqual("class3", ordered[3]);

            c.setDependency("class3", "class6");

            Assert.AreEqual("class3", ordered[2]);
            Assert.AreEqual("class6", ordered[3]);
        }
    }
   }
